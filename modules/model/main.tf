locals {
  ...
  global        = try(local.nso.global, [])
  devices       = try(local.nso.devices, [])
  device_groups = try(local.nso.device_groups, [])
  ...
  templates = { for template in try(local.nso.templates, []) : template.name => template }
  ...
  global_cli_templates_raw = { for device in local.managed_devices :
    device.name => {
      for t in try(local.global.templates, []) : local.templates[t].name => {
        ...
        order   = try(local.templates[t].order, local.defaults.nso.templates.order)
